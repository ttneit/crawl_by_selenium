...
def process_1_ad(browser_ad,full_data:dict ) : 
    ...
    features_name = {'Diện tích':'size','Mức giá':'price_string','Hướng nhà':'direction','Hướng ban công':'balcony_direction','Số phòng ngủ':'bedrooms','Số toilet':'toilets','Pháp lý':'duty','Nội thất':'furniture','Đường vào':'road','Mặt tiền':'frontage'}
    
    for feature in features : 
        name = feature.text.split('\n')[0]
        if name not in features_name.keys() : continue
    ...
    for feature in ad_features : 
        name = feature.text.split('\n')[0]
        if name not in ad_feature_names.keys() : continue
...
def main(browser) :
    ...
    full_data = {
        ...
        'start_date': [], 'end_date': [], 'type': [], 'id': [] ,'road' : [],'frontage' :[]
    }
...
if __name__ == "__main__" : 
    ...
    browser.get("https://batdongsan.com.vn/ban-nha-dat-tp-hcm")
    ...
    df.to_csv("GiaNha.csv",index=False)